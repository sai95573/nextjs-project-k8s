...
export default async function handler(
  ...
  try {
    // Find the user by mobile number and check if registered
    const user = await prisma.users.findFirst({
      where: { mobile, is_registered: true },
      select: { id: true, name: true, mobile: true, is_registered: true },
    });

    // If user does not exist
    if (!user) {
      return res.status(401).json({ message: "Unauthorized: User not registered. Please register before logging in." });
    }
